...
const CONFIG = {
    ...
    strengthLevels: {
        ...
        strong: { min: 50, max: 75, color: '#3b82f6', label: 'Good' },
...
// ===== DOM ELEMENTS =====
// Elements will be initialized in initEnhanced() after DOM is ready
let elements = {};
...
function loadExternalModules() {
    ...
    if (typeof passwordValidator === 'undefined') {
        console.warn('PasswordValidator module not loaded');
    }

    // Load analytics module
    if (typeof passwordAnalytics === 'undefined') {
        console.warn('PasswordAnalytics module not loaded');
    }

    // Load animations module
    if (typeof uiAnimations === 'undefined') {
        console.warn('UIAnimations module not loaded');
    }

    // Load data manager module
    if (typeof dataManager === 'undefined') {
        console.warn('DataManager module not loaded');
    }

    // Make modules globally available
    window.passwordValidator = passwordValidator;
    window.passwordAnalytics = passwordAnalytics;
    window.uiAnimations = uiAnimations;
    window.dataManager = dataManager;
}

// ===== ENHANCED PASSWORD GENERATION =====
function generatePasswordEnhanced(length = 16, options = {}) {
    const password = generatePassword(length);

    // Track analytics
    if (window.passwordAnalytics) {
        window.passwordAnalytics.trackPasswordGeneration(password, options);
    }

    // Validate password
    if (window.passwordValidator) {
        const validation = window.passwordValidator.validatePassword(password);
        if (!validation.isValid) {
            console.warn('Generated password failed validation:', validation.errors);
        }
    }
...
function displayPasswordsEnhanced(passwords) {
    if (window.uiAnimations) {
        window.uiAnimations.animatePasswordGeneration(elements.resultsContainer);
    }

    // Use original display function with enhancements
    setTimeout(() => {
        displayPasswords(passwords);

        // Add hover effects to password items
        if (window.uiAnimations) {
            const passwordItems = elements.resultsContainer.querySelectorAll('.result-item');
            passwordItems.forEach(item => {
                window.uiAnimations.addHoverEffect(item, 'lift');
                window.uiAnimations.addRippleEffect(item.querySelector('.copy-password'));
                window.uiAnimations.addRippleEffect(item.querySelector('.save-password'));
            });
        }
    }, 300);
}
...
function setupEnhancedEventListeners() {
    ...
    if (window.dataManager) {
        ...
        exportJSONBtn.textContent = 'Export Data';
        exportJSONBtn.addEventListener('click', () => {
            ...
            window.dataManager.downloadFile(data, window.dataManager.generateBackupFilename(), 'json');
        });

        const exportCSVTxt = document.createElement('button');
        exportCSVTxt.className = 'btn btn-secondary';
        exportCSVTxt.textContent = 'Export CSV';
        exportCSVTxt.addEventListener('click', () => {
            const data = {
                passwords: state.passwords,
                history: state.history,
                stats: state.stats
            };
            window.dataManager.downloadFile(data, window.dataManager.generateBackupFilename(), 'csv');
        });
        ...
        if (resultsHeader) {
            ...
            exportContainer.appendChild(exportCSVTxt);
            resultsHeader.appendChild(exportContainer);
        }

        // Import functionality
        const importInput = document.createElement('input');
        importInput.type = 'file';
        importInput.accept = '.json,.csv,.txt';
        ...
        importInput.addEventListener('change', async (e) => {
            if (e.target.files.length > 0) {
                ...
                try {
                    const result = await window.dataManager.importFromFile(file, format);
                    if (result.history && result.history.length > 0) {
                        state.history = [...state.history, ...result.history];
                        saveHistory();
                        updateHistoryDisplay();
                        showToast(`Imported ${result.history.length} passwords`, 'success');
                    }
                } catch (error) {
        ...
        if (resultsHeader) {
            resultsHeader.appendChild(importBtn);
    ...
    elements.generateBtn.addEventListener('click', function() {
        if (window.uiAnimations) {
            window.uiAnimations.addRippleEffect(this);
            window.uiAnimations.bounce(this);
        }
        ...
        let loadingSpinner;
        if (window.uiAnimations) {
            loadingSpinner = window.uiAnimations.showLoadingSpinner(elements.resultsContainer, 'Generating secure passwords...');
        }

        // Generate passwords with enhanced function
        state.passwords = [];
        for (let i = 0; i < quantity; i++) {
            const options = {
                customChars: elements.customSet.classList.contains('active'),
                excludeSimilar: elements.excludeSimilar.checked,
                noDuplicates: elements.noDuplicates.checked,
                sequentialCheck: elements.sequentialCheck.checked,
                pronounceable: elements.pronounceable.checked
            };
            state.passwords.push(generatePasswordEnhanced(length, options));
        }

        // Hide loading and display results
        setTimeout(() => {
            if (loadingSpinner) loadingSpinner.hide();
            displayPasswordsEnhanced(state.passwords);
            ...
            if (window.uiAnimations && quantity > 0) {
                window.uiAnimations.celebrate(elements.generateBtn, '#10b981');
            }
    ...
    elements.copyAll.addEventListener('click', function() {
        if (window.uiAnimations) {
            window.uiAnimations.addRippleEffect(this);
        }
        ...
        if (state.passwords.length === 0) {
            ...
            if (window.uiAnimations) {
                window.uiAnimations.shake(this);
            }
            return;
        }
    ...
    elements.saveAll.addEventListener('click', function() {
        if (window.uiAnimations) {
            window.uiAnimations.addRippleEffect(this);
        }
        ...
        if (window.uiAnimations) {
            window.uiAnimations.celebrate(this, '#3b82f6');
        }
...
function initEnhanced() {
    ...
    if (window.uiAnimations) {
        ...
        document.querySelectorAll('.btn').forEach(btn => {
            window.uiAnimations.addHoverEffect(btn, 'lift');
            window.uiAnimations.addRippleEffect(btn);
        });

        // Add fade-in animations to sections
        document.querySelectorAll('.card').forEach((card, index) => {
            window.uiAnimations.fadeIn(card, index * 200);
        });
        ...
        setTimeout(() => {
            ...
            statValues.forEach(stat => {
                ...
                window.uiAnimations.animateCounter(stat, targetValue, 1000);
            });
        }, 1000);
    }

    // Enhanced preview updates
    const originalUpdatePreview = updatePreview;
    updatePreview = function() {
        originalUpdatePreview();

        // Add typing animation to preview
        if (window.uiAnimations) {
            const previewText = elements.previewDisplay.querySelector('.password-text');
            if (previewText) {
                const password = previewText.textContent;
                window.uiAnimations.typeWriter(previewText, password, 30);
            }
        }
    };

    // Enhanced strength analysis
    const originalUpdateStrengthAnalysis = updateStrengthAnalysis;
    updateStrengthAnalysis = function(passwords) {
        originalUpdateStrengthAnalysis(passwords);
        ...
        if (window.uiAnimations) {
            ...
            const strengthColor = getStrengthColor(avgScore);
            window.uiAnimations.animateStrengthMeter(strengthFill, avgScore, strengthColor);
        }
    };
}
